	...
	for i=1,Nsample do
		for chr=1,44 do
			...
			for j=1,50 do
				...
				test.Naive[j*10+i]=test.Naive[j*10+i]+torch.abs(temp_break-1)
				end
			end
		end
		test.Naive[j*10+i]=math.min(test.Naive[j*10+i],train.step_sample+20)
	end
	for i=1,Nsample do
		if (torch.sum(train.state[i])/2200>1.7) then
			...
			for chr=1,44 do
				...
				for j=1,50 do
					...
					temp_break=temp[j]
					test.Naive2[j*10+i]=test.Naive2[j*10+i]+torch.abs(temp_break)
					end
				end
			end
		...
		for chr=1,44 do
			...
			for j=1,50 do
				if(not (train.state[i][{temp_allele+1,(chr-1-temp_allele*22)*50+j,1}]==temp_break) )then
				temp_break=train.state[i][{temp_allele+1,(chr-1-temp_allele*22)*50+j,1}]
				test.Naive2[j*10+i]=test.Naive2[j*10+i]+torch.abs(temp_break-temp_ave)
				end
			end
			test.Naive2[j*10+i]=test.Naive2[j*10+i]+torch.abs(temp_ave-1)
		end
		test.Naive2[j*10+i]=math.min(test.Naive2[j*10+i],train.step_sample+20)
	end
end
