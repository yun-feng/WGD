...
chrom_extract=function(cnp,chrom)
	if chrom<=1 then
		...
		return cnp[{{},{chrom_width*(chrom-1)+1,chrom_width*(chrom)},{}}];
	end
end
...
LoadData=function(flag)
	...
	train.ChrA=torch.floor(torch.rand(train.state:size(1))*(22))+1;
	...
	for i=1,train.Reward:size(1) do
		...
		--elseif train.ChrA[i]>1 then
			startL=chrom_width*(train.ChrA[i]-1)+train.StartL[i];
			endL=chrom_width*(train.ChrA[i]-1)+train.End[i];
